...
class SameSizeCoordsBatch(object):
    ...
    def __init__(self, allowed_atom_num:Sequence[int], dtype ):
        ...
        self.atom_indices    = None  # tensor nconf,nAtoms  with atomic indices
        self._at_index_list  = []
...
#         self.addConformer(xyz, atomTypes)
    ...
    def addConformer(self, xyz:np.ndarray, atomic_nums:np.ndarray):
        ...
        ani2x_code = {1: 0, 6: 1, 7: 2, 8: 3, 9: 4, 16: 5, 17: 6}
        ...
        self._at_index_list.append([ani2x_code.get(x) for x in atomic_nums])
        self.n_confs += 1
    ...
    def collectConformers(self, requires_xyz_grad, device):
        ...
        self.atom_indices = torch.tensor(self._at_index_list, dtype=torch.int64)
        if device is not None:
            ...
            self.atom_indices = self.atom_indices.to(device=device, non_blocking=True)
        ...
        del self._coords_list, self._at_type_list, self._at_index_list
